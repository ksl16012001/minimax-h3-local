...
Log "FL2VA=$fl2Dit steps=$fl2Steps | Ref2VA=$refDit steps=$refSteps | ${W}x${H} x $F frames"
...
if (-not (Test-Path "$out\shot2.webm")) {
  ...
  & .\run_max.ps1 -Dit $refDit -Steps $refSteps -Sampler er_sde -W $W -H $H -Frames $F -Seed ($seed+1) -Prompt $p2 -RefImg @($prodRef) -Out "story15\shot2.webm"
}
...
if (-not (Test-Path "$out\shot3.webm")) {
  ...
  & .\run_max.ps1 -Dit $refDit -Steps $refSteps -Sampler er_sde -W $W -H $H -Frames $F -Seed ($seed+2) -Prompt $p3 -RefImg @($charRef, $prodRef) -Out "story15\shot3.webm"
}
...
Log "assembling"
foreach ($i in 1..3) { & $ff -hide_banner -loglevel error -y -i "$out\shot$i.webm" -c:v libx264 -crf 16 -preset slow -pix_fmt yuv420p -r 24 -c:a aac -b:a 192k -ar 32000 "$out\shot$i.mp4" }
