...
REM  setup.bat            -> recommended set (~28 GB): Q3 DiT + Q2 text encoder + VAEs + turbo LoRA
REM  setup.bat fast       -> minimal set   (~25 GB): Q2 DiT + Q2 text encoder + VAEs
REM  setup.bat max        -> everything    (~140 GB): + Q8 DiTs (FL2VA, Ref2VA), Q4 text encoder,
REM                                                    Ref2V LoRA, bf16 DiT for LoRA baking
REM  Requirements: Windows 10/11, NVIDIA GPU (8 GB VRAM min, tested RTX 3070), Python 3.10+, ~32 GB RAM.
REM  Run from the folder you want as project root (models/, bin/, outputs/ are created here).
REM ============================================================================
set "TIER=%~1"
if "%TIER%"=="" set "TIER=recommended"
set "ROOT=%~dp0"
set "ROOT=%ROOT:~0,-1%"
...
python -m pip install -q -U huggingface_hub imageio-ffmpeg safetensors gguf || (echo [ERROR] pip install failed & exit /b 1)
python -c "import torch" 2>nul || echo [NOTE] torch not installed: only needed for bake_lora.py (pip install torch).
...
if not exist "%ROOT%\bin\sd-cli.exe" (
  curl -L --fail --retry 8 --retry-delay 5 --retry-all-errors -o "%ROOT%\bin\sdcpp-cuda12.zip" "%SDCPP_BASE%/sd-%SDCPP_TAG:master-=%-bin-win-cuda12-x64.zip" || goto :dlfail
  curl -L --fail --retry 8 --retry-delay 5 --retry-all-errors -o "%ROOT%\bin\cudart.zip" "%SDCPP_BASE%/cudart-sd-bin-win-cu12-x64.zip" || goto :dlfail
...
set "HF=python -m huggingface_hub.cli.hf"
hf --help >nul 2>nul && set "HF=hf"
...
echo   .\run_max.ps1 ...                                max-quality render (Q8, 768p, needs 'max' tier)
